...
- **Single source of truth: `src/config/tools.ts`.** It holds `formats` and `tools`
  (slug, from, to) and composes per-locale copy from the language packs. Every tool page, the home page list,
...
- Locales: `en` (default, no prefix), `de`, `ja`, `es`, `ko`, `fr`, `it`, `pt` under `/<locale>/`.
- Slugs are English and identical in every locale (`/de/gpx-to-kml`). Do not localize slugs.
- All copy for a locale lives in one language pack, `src/i18n/locales/<locale>.ts` (UI strings,
  format blurbs, common FAQ, every tool's title/description/FAQ). `tools.ts` composes them.
  A missing string, format or tool in any pack is a compile error, never a silent English fallback.
  Adding a locale: `LOCALES` + `LOCALE_NAMES` in `site.ts`, a pack file, an entry in `packs.ts`.
- Korean text gets `word-break: keep-all` (BaseLayout); CJK descriptions stay ≤ 120 chars.
